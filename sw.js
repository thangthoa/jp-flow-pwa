...
a&&a.forEach(e=>{e(t,r)})})(t,r,e)})
self.addEventListener("install",e=>{e.waitUntil(self.skipWaiting())}),self.addEventListener("activate",e=>{const t=["dg-cache-v66"]
e.waitUntil(caches.keys().then(e=>Promise.all(e.map(e=>{if("dg-cache-v66"!==e||!t.includes(e))return caches.delete(e)}))).then(()=>self.clients.claim()))}),self.addEventListener("fetch",function(e){e.respondWith(caches.match(e.request).then(function(t){return t||fetch(e.request)}))})},"2KUI":function(e,t,r){"use strict"
